...
class UniformAffineQuantizer(nn.Module):
    ...
    def find_params(self, x, weight=False):
        ...
        if not isinstance(self.delta, torch.Tensor):
             self.delta = torch.tensor(self.delta).to(x.device)
        if not isinstance(self.zero_point, torch.Tensor):
             self.zero_point = torch.tensor(self.zero_point).to(x.device)
        self.inited = True

    def quantize_gptq(self, x):
        # x is weight tensor
        x_int = round_ste(x / self.delta) + self.zero_point
        if self.sym:
            x_quant = torch.clamp(x_int, -self.n_levels - 1, self.n_levels)
