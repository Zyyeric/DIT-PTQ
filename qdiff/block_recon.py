...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor,
    ...
    :param sequential: Sequential BRECQ optimization
    """
    ...
    if not act_quant:
        ...
        if hasattr(block, 'attn1'):
            ...
            if hasattr(block, 'attn2') and block.attn2 is not None:
                opt_params += [block.attn2.act_quantizer_q.delta,
            ...
            if block.attn2 is not None and block.attn2.act_quantizer_w.n_bits != 16:
    ...
    if optimizer is not None:
        for i in range(iters):
            ...
            if (i + 1) % log_interval == 0 or i == 0:
                ...
                logger.info(f"  Block recon iter {i+1}/{iters} | loss={err.item():.6f} | "
